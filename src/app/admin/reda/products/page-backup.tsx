...
export default function ProductsAdmin() {
  ...
  const [imageUploadMethod, setImageUploadMethod] = useState<ImageUploadMethod>("file");
  const [mainImageFile, setMainImageFile] = useState<File | null>(null);
  const [mainImagePreview, setMainImagePreview] = useState<string>("");
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);
  ...
  // Edit mode states
  const [editingProduct, setEditingProduct] = useState<Product | null>(null);
  const [editColorVariants, setEditColorVariants] = useState<ColorVariantData[]>([]);
  const [editMainImageFile, setEditMainImageFile] = useState<File | null>(null);
  const [editMainImagePreview, setEditMainImagePreview] = useState<string>("");
  const [editImageUploadMethod, setEditImageUploadMethod] = useState<ImageUploadMethod>("file");
  const [isEditSubmitting, setIsEditSubmitting] = useState(false);
  const editFileInputRef = useRef<HTMLInputElement>(null);
        ...
        {products.map((product) => (
          <div key={product.id} className="p-4 bg-white rounded-lg shadow">
            ...
            <p className="text-gray-600 mb-2">
              Stock:{" "}
              <span
                className={product.stock <= 0 ? "text-red-600 font-bold" : ""}
              >
